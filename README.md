...
- 🧹 **«Убрать купленное»** — кнопка в прогресс-баре активного списка. Появляется, как только есть хотя бы один купленный товар (в т.ч. когда куплены все — служит ручным выходом, если авто-архив не сработал). Купленные переезжают в новый архивный список, активный остаётся только с непокупленными. История покупок сохраняется в архиве.
- 🗂 **Архив** — список архивных карточек с датой+временем создания. Внутри карточки «Добавить в список» открывает шторку: выбор списка-назначения и подмножества товаров (можно снять часть) перед возвратом в активный список (всё снимается с галочки); либо «Удалить список» с подтверждением.
