...
1. **Учётки.** Бот через [@BotFather](https://t.me/BotFather) → `BOT_TOKEN`.
   `OPENAI_API_KEY` — на https://platform.openai.com/.
   Свой Telegram user_id (например, через [@userinfobot](https://t.me/userinfobot)) — в `ALLOWED_USER_IDS`.
...
3. **Прод (Docker + HTTPS).** Telegram Mini App требует HTTPS-домена для
   `webapp`-контейнера. Поднять контейнеры и поставить перед `shopping-list-webapp-1:8000`
   reverse-proxy с TLS (Caddy / Traefik / Nginx), проксирующий корень `/`.
   ```bash
