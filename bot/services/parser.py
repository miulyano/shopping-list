...
def _normalize_qty(value: Any) -> Optional[str]:
...
SYSTEM_PROMPT = (
    ...
    "\n"
    "Если пользователь указал только число без единицы измерения — добавь подходящую "
    ...
    "  - упаковки (макароны, печенье, чипсы, йогурт в стаканчиках, конфеты в коробке) → уп.\n"
    "Выбирай единицу по здравому смыслу: «молоко 1» → 1 л, «сахар 1» → 1 кг, "
    "«сахар 500» → 500 г, «яблоки 5» → 5 шт. "
    "Если пользователь явно указал единицу — не меняй её. "
...
async def parse_text(text: str) -> list[ParsedItem]:
    ...
    for i in data.get("items", []):
        ...
        if not name:
            continue
        out.append(ParsedItem(name=name, qty=_normalize_qty(i.get("qty"))))
    return out
